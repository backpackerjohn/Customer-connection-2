...
- `src/App.tsx` — owns ALL state and effects. Routes between six views: `dashboard`, `today`, `customers`, `profile`, `settings`, `bulk-intake`. The default initial view is now `'customers'` (meaning dealers land on the customer list directly). State includes auth, customers list, current customer, notes, save status, AI overlay open/closed, Test Drive packet generation, Sold packet generation, trade valuation, plus error toasts for each. Pure prop drilling — no Context, no Redux, no React Query.
...
- `src/services/notesService.ts` — chokepoint for the `notes` subcollection. Implements `subscribeToAllNotes(uid, onChange, onError)` using the Firestore `collectionGroup('notes')` query where `authorId == uid` to enable cross-customer recall search. `App.tsx` maintains a `Map<customerId, string[]>` of note contents (`notesByCustomer`) and passes it down so note search is fully functional. In production, this requires a single-field collectionGroup index on `authorId`.
...
- `src/services/aiService.ts` — `processCustomerChat(userInput, currentData, chatHistory, image?, intent = 'other')`. Powers the single-customer chat overlay. Owns its own response schema, system instruction, and response parsing. **Photo routing** (see `src/lib/captureIntent.ts`): when `intent` is a section (`trade`/`vehicle`/`insurance`/`license`, set by the overlay's chip row or a section camera button) the call uses a scoped prompt + scoped schema built from that section's whitelist. When `intent` is `'other'` and a photo is attached, the model classifies the photo in `documentType` (`license`/`insurance`/`trade_vehicle`/`new_vehicle`/`vehicle`/`window_sticker`/`other`), with the dealer's words winning over the picture ("here's his trade" → `trade_vehicle`), and `filterByIntent` drops everything outside that document's whitelist. `documentType: 'vehicle'` means a VIN/vehicle photo where nobody said trade vs new: the fields are moved into `pendingVehicle` (nothing written) and the overlay asks with two chips. `documentType: 'other'` and text-only messages keep the unfiltered passthrough. Insurance cards return `insuredVehicle` separately so the overlay can offer it as the trade-in. Post-processes extracted fields to force `hasTradeIn = true` when any trade field is returned, and BOTH `stillOwe = true` AND `hasTradeIn = true` when any financial field (lienholder/payoffAmount/monthlyPayment/monthsRemaining) is returned (ensures fields hidden behind profile UI toggles are visible; mirrors how Bulk Intake forces hasTradeIn).
...
- `src/lib/dateUtils.ts` — pure date utilities (`formatDateISO`, `parseDateString`, `getDaysDiff`), imported by `src/lib/customerSearch.ts`, `src/lib/reminders/engine.ts`, and `src/views/CustomersView.tsx`. This is the single home for these helper functions; duplicate copies were removed.
...
- `src/lib/customerSearch.ts` — pure filter & search engine (takes `today` as a parameter, has no Firestore bindings). Owns `resolveVehicles(customer)` (groups vehicle values based on status: lead/inactive → owns=trade*, wants=vehicle*; sold → owns=vehicle*, wants=[]), and `applyFilters(customers, criteria, today, notesByCustomer?)` which evaluates multi-factor facets and recall search terms, returning `{matched, unclassified}` lists. Covered by tests in `customerSearch.test.ts`.
- `src/lib/timing.ts` — `timed(label, fn)` performance instrumentation helper.
- `src/lib/leadSource.ts` — the single source of truth for the Lead Source picker taxonomy (parent groups Walk-In / CRM / Referral / Social and their children, with labels), plus `findLeadSourceGroup()`. Typed against `Customer['leadSourceType']`. Also exports `LEAD_SOURCE_FLAT_OPTIONS` (flattened parents and children labeled "Parent › Child") for compact dropdowns like the Bulk Intake row picker.

### Components
- `src/components/` — atomic UI primitives: `InputField`, `Toggle`, `StatusBadge`, `SaveStatusIndicator`, `MenuButton`, `SubButton`, `NavItem`, `NavIconButton`, `AIChatOverlay` (attach-then-send: Upload, Camera, and pasted images become pending thumbnails with an optional section chip row; several photos per message are allowed, each goes out as its own parallel `processCustomerChat` call with the same typed text; replies carry a deterministic "License → Customer Info: 9 fields filled" summary line, an "Add … as trade?" chip for insurance cards, and a "Trade-in / New vehicle" chip pair for ambiguous vehicle photos; props `initialIntent` + `autoOpenPicker` let a section camera button open it pre-scoped), `DocumentTray` (the six-slot photo tray from `CAPTURE_SLOTS` in `src/lib/captureIntent.ts`: License / Insurance / New Vehicle / Trade-in / Payoff / Other. The slot IS the tag: tap a tile to add one or more photos via the native chooser or file dialog, drag files onto tiles, paste into Other, tap a thumbnail to "Move to" another slot, then one Extract hands the batch to `AIChatOverlay` as a `ChatSeed` and the chat sends each photo as its own parallel scoped call under that slot's whitelist. Nothing is written until Extract. License/Insurance tiles show "On file" when `dlImageUrl`/`insuranceImageUrl` exist. Opened from the profile's AI menu → Photos, or from a section camera icon with that slot's picker opened immediately), `RescheduleButton`, `TextedCheckbox`, `TradeEquityPanel`, `VinLookupButtons`, `EditableChip` (click-to-expand chip with popover, used for status + leadSourceType on Bulk Intake rows), `CopyButton` (click-to-copy icon button with checkmark flash; used on the Today page for customer name + phone + rendered template text), `SegmentedControl` (always-visible fast segmented control pill for Status), `ChipSelect` (flat row of single-select chips with clearance support), `LeadSourceChips` (two-level hierarchal grouped chip picker).

### Views
- `src/views/LoginView.tsx` — Google sign-in screen.
- `src/views/DashboardView.tsx` — placeholder welcome page only. It accepts but does not use its props (`(_props: Props) + void _props;`) to satisfy `noUnusedLocals` so its existing call site in `App.tsx` continues to typecheck. The actual customer list lives in `src/views/CustomersView.tsx`.
- `src/views/CustomersView.tsx` — Desktop-first power-filter and saved views directory. Allows multi-factor filtering across multiple facets: status (with fast All/Unsold/Sold/Inactive buttons), leadSourceType, vehicle scope (either/owns/wants) + make/model fields, car age (any, new, mid, old, custom), time since purchase, and recency of contact or addition (including custom date ranges). Free-text "recall" search box queries multiple fields and customer note contents (via `notesByCustomer` prop). Displays a "Couldn't classify (N)" disclosure only when a vehicle facet is active to surface unparsed data. The filter panel is hidden by default on load, toggled via a dynamic "Filter & Search"/"Hide Filters" button. Customer cards render as a 2-column grid on desktop/tablet to provide a wider viewport for upcoming card content. Also includes the shared Template Bar (persists `todayTemplate` and `latestModelYear` in localStorage) which renders copyable personalized templates, a "Push to Today" action writing to `todayPinnedIds` in localStorage, and "Saved Smart Views" persisting filter configurations (not customer IDs) to `savedCustomerFilters` in localStorage. Only the Texted checkmarks roll relationship cadence; copying, templating, and pinning are inert.
  - **Contextual Bottom Action Bar**: A `MenuButton`-based fixed bottom bar (`fixed bottom-0 left-0 right-0 md:left-64`, height animates 90px closed ↔ 180px open via Framer Motion spring; backdrop `bg-black/5` closes on click) renders on the Customers view, replacing and hiding the global mobile nav in `src/App.tsx`. Features five main buttons left-to-right (Menu/LayoutDashboard, Filters/SlidersHorizontal, Sort/ArrowUpDown, New/Plus, Push/ArrowUpRight) with local `activeMenu: string | null` state. Clicking Menu or Sort opens slide-up sub-menus populated with decorative `SubButton` options (Menu has Dashboard, Today, Bulk Upload, Settings, and Sign Out sub-buttons; Sort has Alphabetical, Date Added, and Latest Viewed sub-buttons). Filters, New, and Push main buttons, and all sub-buttons, are decorative/inert this round (to be implemented in later commits). Uses the same `MenuButton` + `SubButton` pattern as the profile's bottom action bar.
- `src/views/TodayView.tsx` — reminder dashboard. Aggregates `getDueReminders(...)` across all customers. Each row has a `TextedCheckbox` (close reminders + roll cadence), a `RescheduleButton` (push to a specific date with a reason), and inline `CopyButton`s for customer name + phone. A template bar at the top accepts a dealer-pasted text template; when non-empty, each row renders a personalized preview via `renderTemplate` with a `CopyButton`. Template text and current model year persist in `localStorage` (`todayTemplate` + `latestModelYear`), NOT in Firestore. Each row also has an inline expandable Notes panel — a collapsed "Notes" strip below the row that, when clicked, opens an inline panel mirroring the profile's `TimelineNotesSection` (add-note input + scrollable notes feed). Only one row is expanded at a time; a single `subscribeToNotes` listener is active for the expanded customer, torn down on collapse. ALSO features a "Pinned from Customers" section rendered from `todayPinnedIds` in `localStorage`, which sits completely outside the reminder engine (does not mutate `nextCadenceDue`, can be cleared at once or individually toggled off).
- `src/views/CustomerProfileView.tsx` — sticky header (displays always-visible Status, Lead Source, and Contact controls using SegmentedControl, LeadSourceChips, and ChipSelect respectively, replacing the old popover-style EditableChips), the 6 form sections (composed from `src/views/profile/`; Customer Info, Insurance, New Vehicle, and Trade-in each render a header camera icon wired to `onOpenDocumentTray(slot)`), and the bottom action bar (App menu / AI menu with Chat, Photos, Test Drive, Sold / Insights menu / Customer menu).
...
### Rules & schema
- `firestore.rules` — security rules. Strict; rejects unknown fields on update, oversized strings, identity spoofing. Includes an additive collectionGroup read rule `match /{path=**}/notes/{noteId}` allowing list queries only when `resource.data.authorId == request.auth.uid` (used for note recall search without modifying `isValidCustomer` or risking the 1000-expression eval budget).
...
- Bulk-set controls also include "Set lead source for all" (using `LeadSourceChips`) and "Set contact for all" (using `ChipSelect`), which apply to every row and are hidden after commit.
- Row-level overrides are preserved: the per-row Lead Source chip uses `LEAD_SOURCE_FLAT_OPTIONS` for the full taxonomy, and a new per-row Contact chip uses `EditableChip`.
- Commit path is unchanged: `leadSourceType` and `contactChannel` persist because the commit payload spreads the row's customer object.
...
- `contactChannel: 'text' | 'crm-text' | 'email' | 'snapchat' | 'facebook'` — optional dealer-preferred communication channel for this customer. Settable in both profile (via `ChipSelect`) and Bulk Intake (bulk-set with `ChipSelect` + per-row override with `EditableChip`). NEVER extracted by AI.
...
- Photo extraction whitelists live in `INTENT_FIELDS` (`src/lib/captureIntent.ts`), one per tray slot: `license`, `insurance`, `vehicle`, `trade`, `payoff`. A photo sent under a slot can only ever write that slot's fields; `filterByIntent` enforces it after the model responds regardless of what the scoped schema allowed. The `payoff` slot writes `stillOwe`/`lienholder`/`payoffAmount`/`monthlyPayment`/`monthsRemaining` and the existing post-processor flips `hasTradeIn`/`stillOwe` on. Adding a slot means: extend `CaptureIntent`, add a whitelist to `INTENT_FIELDS`, a label to `INTENT_SECTION_LABEL`, an entry to `CAPTURE_SLOTS`, prompt docs in `aiService` (`INTENT_FIELD_DOCS` / `INTENT_CONTEXT` / `INTENT_EXTRAS`), a `DocumentType` value + `intentFromDocumentType` case, and the icon/tint maps in `DocumentTray.tsx` and `INTENT_LABELS` in `AIChatOverlay.tsx`.
- Only the "Texted" checkmark (via `TextedCheckbox`) rolls the cadence and updates `lastContactedAt` (on both TodayView and CustomersView). Tools under `CustomersView` like Template Bar copying, template rendering, and "Push to Today" pins are deliberately inert and never roll cadence or set `lastContactedAt`.
- "Push to Today" state (`todayPinnedIds` in `localStorage`) and "Saved filters" state (`savedCustomerFilters` in `localStorage`) are strictly local and never modify relationship cadence or write to Firestore.
- To avoid blowing the Firestore rules 1000-expression evaluation budget, we did not touch `isValidCustomer` or any of the existing write validation rules to facilitate note-search capability. Instead, we added an independent, scoped collectionGroup read rule.
- `src/views/DashboardView.tsx` is intentionally a placeholder welcome page with no logic — it accepts props it doesn't use via `(_props: Props) + void _props;` to satisfy `noUnusedLocals` without an `eslint-disable` comment. The default initial view in `src/App.tsx` is set to `'customers'` so a signed-in dealer lands directly on the customer list rather than this empty Dashboard placeholder.
