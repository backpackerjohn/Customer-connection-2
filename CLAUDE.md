...
- `leadSourceType` — optional structured source tag. The **pickable** values are exactly five: `'walk-in' | 'crm' | 'vep' | 'dealer-wizard' | 'fb-marketplace'`. The TypeScript union ALSO carries five **legacy** values (`'referral' | 'showroom' | 'phone' | 'web' | 'other'`) so existing customer docs continue to type-check — these don't appear in the picker, and any customer holding one will see the placeholder ("— Not set —") in the chip until the dealer re-picks. No migration. Coexists with the free-text `leadSource` (raw CRM-extracted string preserved for context).
- **AI extraction is restricted**: only `'walk-in'`, `'crm'`, and `'vep'` are auto-derivable. `'dealer-wizard'` and `'fb-marketplace'` are dealer-only via the chip. `aiService.processCustomerChat` extracts only those three values. `bulkIntakeService.extractBulkCustomers` deliberately does NOT extract `leadSourceType` at all — the bulk path uses the `deriveLeadSourceType` heuristic in `BulkIntakeView.tsx` + dealer chip override. Intentional exception to the "both extractors must stay in sync" gotcha.
...
- `contactChannel: 'text' | 'crm-text' | 'email' | 'snapchat' | 'facebook'` — optional dealer-preferred communication channel for this customer. Profile-only, NEVER extracted by AI. Picker lives in the same top chip row on the profile (third chip after Status and Lead Source). Not surfaced in Bulk Intake.
