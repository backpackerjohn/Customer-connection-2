...
- `src/lib/reminders/engine.ts` — `getDueReminders(customer, today, config)`, `recordContact(...)`, `computeNextCadenceDue(...)`, `rollNextCadence(...)`, `ReminderKind` type union. Customer lifecycle: **lead** → (Sold button stamps `purchaseDate`) → **fresh buyer** (7-day window, no combining) → **standard buyer** (3–6 month cadence) → anniversary forever.
- `src/lib/reminders/engine.test.ts` — exhaustive coverage of cadence, fresh buyer follow-ups, referrals, anniversary, holiday combining, manual reminders. The clearest spec for how reminders fire.
...
- `src/views/profile/*Section.tsx` — one file per form card: `CustomerInfoSection`, `InsuranceSection`, `NewVehicleSection`, `TradeInSection`, `GoalsSection`, `TimelineNotesSection`. Each takes `{ customer, onChange }` (`TimelineNotesSection` takes extra `notes`/`newNote`/`onAddNote` props; `TradeInSection` takes valuation props).
...
- Customer lifecycle: **lead** (cadence 26–30 days, configurable; reminder kind `cadence`) → after the Sold button stamps `purchaseDate` → **fresh buyer** (7-day window, no combining; fires `followUp24h`, `referral48to72h`, plus birthday/manual) → after the window → **standard buyer** (cadence 3–6 months; reminder kind `cadence`) → `anniversary` reminder yearly forever. `holiday` reminders combine within a 7-day window.
- `getDueReminders(customer, today, config)` aggregates everything due today for one customer; `TodayView` calls it once per customer and sorts by primary due date.
...
- To add a new `ReminderKind`: extend the union in `src/lib/reminders/engine.ts`, optionally add a knob to `REMINDER_CONFIG`, add a case inside `getDueReminders`, update `REASON_LABELS` in `TodayView.tsx`, allow the new kind in `isValidContact` (in `firestore.rules` AND `DRAFT_firestore.rules`), and add tests in `engine.test.ts`.
