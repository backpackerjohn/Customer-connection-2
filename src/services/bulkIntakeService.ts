...
export async function extractBulkCustomers(image: { inlineData: { data: string; mimeType: string } }): Promise<BulkExtractedRow[]> {
  ...
  const systemInstruction = `
    ...
    - hasTradeIn (boolean) + tradeYear, tradeMake, tradeModel, tradeTrim, tradeMileage, tradeVin: Trade-In (i.e. the vehicle the customer is giving up). A vehicle tagged "(Trade-In)" in the source MUST map here and you MUST set hasTradeIn=true. A vehicle described as "trade", "trade-in", or "giving up" MUST also map here. ALSO: a vehicle listed in a "Currently Owns" / "Garage" / "Current Vehicle" / "Currently Owned" section (with NO explicit "(New)" or "(Used)" tag on that vehicle) MUST map to trade-in fields and you MUST set hasTradeIn=true — these CRM profile sections describe what the customer drives today, which is their trade-in. If MULTIPLE vehicles appear in such a section, pick the one with the NEWEST model year (highest year number) for the trade fields; do NOT split the others into multiple trade entries.
